...
uv_tcp_t *Hub::connect(std::string uri, void *user, std::map<std::string, std::string> extraHeaders, int timeoutMs, Group<CLIENT> *eh) {
    if (!eh) {
    ...
    if (!parseURI(uri, secure, hostname, port, path)) {
        ...
        // eh->fd;
        auto tcpHandle = uS::Node::connect<allocateHttpSocket, onClientConnection>(hostname.c_str(), port, secure, eh, [timeoutMs, user, path, port, hostname, extraHeaders, eh](uS::Socket *socket) {
            HttpSocket<CLIENT> *httpSocket = (HttpSocket<CLIENT>*) socket;
            if (httpSocket) {
